...
function scene:create( event )
	...
	for i = 1, 10 do
		...
		--NommerButtons
        -----------------------------------------------------------------------------------------------------------------------
		nommerButton = display.newText( i, cWidth* (i+2)  / 15, display.contentCenterY -yInset*5, teachersPetFont, 36 )
		nommerButton.no = i
		sceneGroup:insert(nommerButton)
		nommerButtonBack = display.newRect( cWidth * (i+2) / 15, display.contentCenterY -yInset*5, cWidth / 14, cWidth / 14 )
		...
		tick.y = display.contentCenterY -yInset*5.2
		c.xScale = 0.05
		c.yScale = 0.05
		tick.alpha =0 
		...
		if i == 1 then 
			...
			currentDragNommerBar.fill.effect = "filter.grayscale"
	...
	elseif event.phase == "ended" then
		...
		if isOverlapping( event.target, currentTargetNommerBar ) then
			...
			if ( nommersOor > 1 ) then
				...
				zander.yScale =0.35
				zander.y = -yInset*0.5
				local woorde ={"Uitstekend","Puik","Fantasties"}
