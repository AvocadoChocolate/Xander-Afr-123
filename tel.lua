...
local nommerTextTable = { "Een", "Twee", "Drie", "Vier", "Vyf", "Ses", "Sewe", "Agt", "Nege", "Tien" }
...
function scene:create( event )
	...
	homeBtn.y = display.contentCenterY -yInset*5
	homeBtn.xScale = 0.5
	homeBtn.yScale = 0.5
	homeBtn.alpha = 0.8
	...
	for i = 1, 10 do
		local nommerButton = display.newText( i, cWidth* (i+2)  / 15, display.contentCenterY -yInset*5, teachersPetFont, 36 )
		nommerButton.no = i
			
		local nommerButtonBack = display.newRect( cWidth * (i+2) / 15, display.contentCenterY -yInset*5, cWidth / 14, cWidth / 14 )
...
function scene:show( event )
    ...
    elseif phase == "did" then
		...
		goingHome = false
		curDots = display.newGroup()
		sceneGroup:insert(curDots)
		loadDots()
...
loadDots = function()
	local rows = 1
	local totalrows = math.ceil( currentActiveNommerButtonTel.no / 2 )
	curDots:removeSelf()
	curDots = nil
	curDots = display.newGroup()
	whitebg:removeEventListener( "touch", slideListener )
	loadingDots = true
	for i=1,currentActiveNommerButtonTel.no do
		if(math.fmod(i, 2)~= 0 and i == currentActiveNommerButtonTel.no) then
			timer.performWithDelay(_transition_time*i,function()
				if(goingHome==false)then
					local nextNomImage = display.newImage( "bouImages/" .. currentActiveNommerButtonTel.no .. ".png", cWidth - xInset*3 , cHeight/2 - yInset * totalrows/2 + 1.5*yInset*(rows-0.5))
					nextNomImage:scale( cWidth / 1000, cWidth / 1000 )
					curDots:insert(nextNomImage)
				end
			end)
		elseif(math.fmod(i, 2)== 0)then
			timer.performWithDelay(_transition_time*i,function()
				if(goingHome==false)then
					local nextNomImage = display.newImage( "bouImages/" .. currentActiveNommerButtonTel.no .. ".png", cWidth - xInset*3 + xInset, cHeight/2 - yInset * totalrows/2 + 1.5*yInset*(rows-0.5))
					nextNomImage:scale( cWidth / 1000, cWidth / 1000 )
					rows = rows + 1
					curDots:insert(nextNomImage)
				end
	...
	timer.performWithDelay(_transition_time*currentActiveNommerButtonTel.no + 500,function()
...
handleNommerButtonEvent =  function (event)
	...
	if event.target.coupledNommer.isActive == false and loadingDots == false then
...
shiftLeft = function ()
	...
	timer.performWithDelay(_transition_time,loadDots())
	curDierImage:addEventListener( "tap", dierImageClicklistener )
